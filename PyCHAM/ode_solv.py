...
# File Created at 2022-08-01 08:46:29.261932
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((20405+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((11180))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:4300:5] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((3440))
			wall_eff[0:1720:2] = -kw # effect of gas on gas 
			wall_eff[1:1720:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[1720:3440:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[1720+1:3440:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
