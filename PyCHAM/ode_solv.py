...
# File Created at 2021-07-20 17:48:24.901841
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((31542+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((28518))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:9702:33] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((1176))
			wall_eff[0:588:2] = -kw # effect of gas on gas 
			wall_eff[1:588:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[588:1176:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[588+1:1176:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
