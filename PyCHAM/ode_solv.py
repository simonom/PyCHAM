...
# File Created at 2022-08-05 16:49:15.449858
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	# Cw - effective absorbing mass concentration of wall (molecules/cm3) 
	...
	def dydt(t, y): # define the ODE(s)
		...
		lr = 1./1200. # first order loss rate to wall (/s)
		lrnr = 1200. # first order loss rate to wall of non-radical ELVOC and LVOC (/s)
		dd[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-6)*(self.Psat_Pa[0, :]<= 3.8e-2))] -= y[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-6)*(self.Psat_Pa[0, :]<= 3.8e-2))]*(1./(lrnr-0)) 
		dd[(num_comp*num_sb)::][((self.Psat_Pa[0, :]> 3.8e-6)*(self.Psat_Pa[0, :]<= 3.8e-2))] += y[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-6)*(self.Psat_Pa[0, :]<= 3.8e-2))]*(1./(lrnr-0)) 
		dd[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-10)*(self.Psat_Pa[0, :]<= 3.8e-6))] -= y[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-10)*(self.Psat_Pa[0, :]<= 3.8e-6))]*(1./(lrnr+0)) # LVOCs following Sarrafzadeh et al. (2016) 
		dd[(num_comp*num_sb)::][((self.Psat_Pa[0, :]> 3.8e-10)*(self.Psat_Pa[0, :]<= 3.8e-6))] += y[0:num_comp][((self.Psat_Pa[0, :]> 3.8e-10)*(self.Psat_Pa[0, :]<= 3.8e-6))]*(1./(lrnr+0)) # LVOCs following Sarrafzadeh et al. (2016) 
		dd[0:num_comp][self.Psat_Pa[0, :] <= 3.8e-10] -= y[0:num_comp][self.Psat_Pa[0, :] <= 3.8e-10]*(1./lrnr) # ELVOCs following Ehn et al. (2014) 
		dd[(num_comp*num_sb)::][self.Psat_Pa[0, :] <= 3.8e-10] += y[0:num_comp][self.Psat_Pa[0, :] <= 3.8e-10]*(1./lrnr) # ELVOCs following Ehn et al. (2014) 
		dd[self.RO2_indices[:, 1]] -= y[self.RO2_indices[:, 1]]*(lr) # RO2 components following Silvia thesis
		dd[(num_comp*num_sb)+self.RO2_indices[:, 1]] += y[self.RO2_indices[:, 1]]*(lr) # RO2 components following Silvia thesis
		dd[self.comp_namelist.index('HO2')] -= y[self.comp_namelist.index('HO2')]*(lr) # following Silvia thesis 
		dd[(num_comp*num_sb)+self.comp_namelist.index('HO2')] += y[self.comp_namelist.index('HO2')]*(lr) # following Silvia thesis 
		dd[self.comp_namelist.index('SA')] -= y[self.comp_namelist.index('SA')]*(1./120.) # following Silvia email 
		dd[(num_comp*num_sb)+self.comp_namelist.index('SA')] += y[self.comp_namelist.index('SA')]*(1./120.) # following Silvia email 
		dd[self.comp_namelist.index('OH')] -= y[self.comp_namelist.index('OH')]*(4.) # following Silvia email (originally 4 /s)
		dd[(num_comp*num_sb)+self.comp_namelist.index('OH')] += y[self.comp_namelist.index('OH')]*(4.) # following Silvia email (originally 4 /s)
		
		# account for components with continuous gas-phase influx
		dd[[1, 20, 31, 29, 10, 2, 3], 0] += Cinfl_now[:, 0]
		# account for continuous extraction of chamber air
		if (self.wall_on == 1): # if wall on
			df_indx = np.ones((len(dd)-num_comp)).astype('int') # index for estimating dilution factors
			df_indx[H2Oi::num_comp] = 0 # water diluted in water solver 
			df_indx = df_indx==1 # transform to Boolean array 
			dd[0:-num_comp, 0][df_indx] -= y[0:-num_comp, 0][df_indx]*1.*self.dil_fac
		if (self.wall_on == 0): # if wall off
			df_indx = np.ones((len(dd))).astype('int') # index for estimating dilution factors
			df_indx[H2Oi::num_comp] = 0 # water diluted in water solver 
			df_indx = df_indx==1 # transform to Boolean array 
			dd[df_indx, 0] -= y[df_indx, 0]*1.*self.dil_fac
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((12843+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((3516))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:1758:2] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((3516))
			wall_eff[0:1758:2] = -kw # effect of gas on gas 
			wall_eff[1:1758:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[1758:3516:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[1758+1:3516:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			data[jac_wall_indx] += wall_eff
		
		data[jac_extr_indx] -= 1.*self.dil_fac
