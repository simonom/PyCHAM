...
# File Created at 2023-04-25 16:19:38.575859
...
def ode_solv(y, integ_step, rrc, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		dd[[10, 126, 487, 1, 2, 3, 4, 13, 12, 15, 127, 128, 129, 134, 144, 164, 172, 174, 176, 178, 180, 182, 338, 334, 336, 21, 24, 327, 31, 39, 20, 22, 30, 449, 248, 252, 262, 257, 267, 272, 251, 292, 297, 282, 94, 186, 197, 195, 198, 375, 224, 383, 504, 325, 449, 69, 481, 465, 342, 340, 287, 786, 26, 761, 33, 44, 4759, 3547, 504, 1034, 520, 247, 104, 34], 0] += Cinfl_now[:, 0]
		...
		if any(wsb):
			...
			#print(dd_all); import ipdb; ipdb.set_trace()
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((188061+jac_mod_len))
		...
		wall_eff = np.zeros((23336))
		# effect of gas on gas 
		wall_eff[0:11668:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:11668:2] = +kimt[num_asb+wsb, :] 
