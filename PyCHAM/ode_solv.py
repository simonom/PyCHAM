...
# File Created at 2022-09-13 15:33:23.176599
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		dd[[10, 57, 20, 1, 2, 3, 29, 31, 34, 37, 41, 45, 47, 49, 51, 53, 55, 59, 61, 63, 65, 67, 68, 71, 74, 76, 80, 81, 86, 90, 94, 99, 104, 109, 114, 93, 123, 128, 133, 138, 141, 147, 151, 152, 173, 159, 180, 187, 191, 194, 198, 201, 204, 206, 211, 214, 217, 84, 219, 223, 225, 227, 228, 215, 230, 220, 238], 0] += Cinfl_now[:, 0]
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((99539+jac_mod_len))
		...
		part_eff = np.zeros((64429))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:23737:7] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((13564))
		# effect of gas on gas 
		wall_eff[0:6782:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:6782:2] = +kimt[num_asb+wsb, :] 
