...
# File Created at 2025-05-06 12:41:40.899572
...
def ode_solv(y, integ_step, rrc, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		if (self.odsw_flag == 0): # if water solver not used 
			...
			dd[num_comp+H2Oi::num_comp, 0] -= y[num_comp+H2Oi::num_comp, 0]*self.dil_fac
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((5324+jac_mod_len))
		...
		part_eff = np.zeros((4247))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:1507:11] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((548))
		# effect of gas on gas 
		wall_eff[0:274:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:274:2] = +kimt[num_asb+wsb, :] 
