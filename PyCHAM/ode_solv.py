...
# File Created at 2020-10-02 09:30:28.661006
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		Csit[isb, :] = (ymat[isb, :]/csum[isb, :])*Psat[isb, :]*kelv_fac[isb]*act_coeff[isb, :]
	...
	def jac(t,y): # define the Jacobian
		...
		# gas-particle partitioning
		part_eff = np.zeros((196))
		part_eff[0:68:17] = -kimt.sum(axis=0) # effect of gas on gas
		
		# transform particle phase concentrations into
		# size bins in rows, components in columns
		ymat = (y[num_comp:num_comp*(num_asb+1)]).reshape(num_asb, num_comp)
		# total particle-phase concentration per size bin (molecules/cc (air))
		csum = ymat.sum(axis=1)-ymat[:, corei]+ymat[:, corei]*core_diss
		...
		if (Cw>0.):
			wall_eff = np.zeros((16))
			wall_eff[0:8:2] = -kw # effect of gas on gas 
			wall_eff[1:8:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[8:16:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[8+1:16:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
