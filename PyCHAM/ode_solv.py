...
# File Created at 2022-08-10 17:33:55.464312
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((5342+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((2226))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:954:3] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((1272))
			wall_eff[0:636:2] = -kw # effect of gas on gas 
			wall_eff[1:636:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[636:1272:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[636+1:1272:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
