...
# File Created at 2023-08-21 14:32:21.990556
...
def ode_solv(y, integ_step, rrc, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((146197+jac_mod_len))
		...
		part_eff = np.zeros((35136))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:17568:2] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((61488))
		# effect of gas on gas 
		wall_eff[0:26352:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:26352:3] = +kimt[num_asb+wsb, :] 
