...
# File Created at 2022-05-26 13:02:02.113781
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		dd[0:num_comp][((Psat_Pa[0, :]> 3.8e-6)*(Psat_Pa[0, :]<= 3.8e-2))] -= y[0:num_comp][((Psat_Pa[0, :]> 3.8e-6)*(Psat_Pa[0, :]<= 3.8e-2))]*(1./(lrnr-0)) 
		dd[(num_comp*num_sb)::][((Psat_Pa[0, :]> 3.8e-6)*(Psat_Pa[0, :]<= 3.8e-2))] += y[0:num_comp][((Psat_Pa[0, :]> 3.8e-6)*(Psat_Pa[0, :]<= 3.8e-2))]*(1./(lrnr-0)) 
		dd[0:num_comp][((Psat_Pa[0, :]> 3.8e-10)*(Psat_Pa[0, :]<= 3.8e-6))] -= y[0:num_comp][((Psat_Pa[0, :]> 3.8e-10)*(Psat_Pa[0, :]<= 3.8e-6))]*(1./(lrnr+0)) # LVOCs following Sarrafzadeh et al. (2016) 
		dd[(num_comp*num_sb)::][((Psat_Pa[0, :]> 3.8e-10)*(Psat_Pa[0, :]<= 3.8e-6))] += y[0:num_comp][((Psat_Pa[0, :]> 3.8e-10)*(Psat_Pa[0, :]<= 3.8e-6))]*(1./(lrnr+0)) # LVOCs following Sarrafzadeh et al. (2016) 
		dd[0:num_comp][Psat_Pa[0, :] <= 3.8e-10] -= y[0:num_comp][Psat_Pa[0, :] <= 3.8e-10]*(1./lrnr) # ELVOCs following Ehn et al. (2014) 
		dd[(num_comp*num_sb)::][Psat_Pa[0, :] <= 3.8e-10] += y[0:num_comp][Psat_Pa[0, :] <= 3.8e-10]*(1./lrnr) # ELVOCs following Ehn et al. (2014) 
		...
		dd[comp_namelist.index('HO2')] -= y[comp_namelist.index('HO2')]*(lr) # following Silvia thesis 
		dd[(num_comp*num_sb)+comp_namelist.index('HO2')] += y[comp_namelist.index('HO2')]*(lr) # following Silvia thesis 
		dd[comp_namelist.index('SA')] -= y[comp_namelist.index('SA')]*(1./120.) # following Silvia email 
		dd[(num_comp*num_sb)+comp_namelist.index('SA')] += y[comp_namelist.index('SA')]*(1./120.) # following Silvia email 
		dd[comp_namelist.index('OH')] -= y[comp_namelist.index('OH')]*(4.) # following Silvia email (originally 4 /s)
		dd[(num_comp*num_sb)+comp_namelist.index('OH')] += y[comp_namelist.index('OH')]*(4.) # following Silvia email (originally 4 /s)
		
		# account for components with continuous gas-phase influx
		dd[[1, 38, 23, 10, 2, 3], 0] += Cinfl_now[:, 0]
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((17086+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((5128))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:2564:2] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((5128))
			wall_eff[0:2564:2] = -kw # effect of gas on gas 
			wall_eff[1:2564:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[2564:5128:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[2564+1:5128:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
