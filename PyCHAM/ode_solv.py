...
# File Created at 2021-08-25 14:53:53.595077
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((40147+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((24528))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:9198:6] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((6132))
			wall_eff[0:3066:2] = -kw # effect of gas on gas 
			wall_eff[1:3066:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[3066:6132:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[3066+1:6132:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
