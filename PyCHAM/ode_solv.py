...
# File Created at 2021-01-26 17:16:54.679718
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((4370+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((1264))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:632:2] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((1264))
			wall_eff[0:632:2] = -kw # effect of gas on gas 
			wall_eff[1:632:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[632:1264:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[632+1:1264:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
