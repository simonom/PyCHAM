...
# File Created at 2023-04-14 15:25:38.407076
...
def ode_solv(y, integ_step, rrc, 
	...
	# self.jac_den_indx - index of component denominators for Jacobian
	...
	# self.y_rindx_g - index of y relating to reactants for reaction rate 
	...
	# 	calculation
	# self.rr_arr_p_g - index for reaction rates to allow reactant loss
	# 	calculation
	...
	def dydt(t, y): # define the ODE(s)
		...
		# gain of products
		data = rr[self.rr_arr_p_g]*self.pstoi_flat_g # prepare gain values
		# convert to sparse matrix
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((51307+jac_mod_len))
		...
		part_eff = np.zeros((34826))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:12664:8] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((6332))
		# effect of gas on gas 
		wall_eff[0:3166:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:3166:2] = +kimt[num_asb+wsb, :] 
