...
# File Created at 2023-07-03 16:30:39.858065
...
def ode_solv(y, integ_step, rrc, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		if self.odsw_flag == 0: # if water not solved separately
			dd[H2Oi, 0] += self.Cinfl_H2O_now
	...
	def jac(t, y): # define the Jacobian
		...
		wall_eff = np.zeros((28))
		# effect of gas on gas 
		wall_eff[0:12:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:12:3] = +kimt[num_asb+wsb, :] 
