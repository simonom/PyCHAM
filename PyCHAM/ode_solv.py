...
# File Created at 2022-11-01 15:55:28.662376
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		dd[[10, 126, 487, 1, 2, 3, 4, 13, 12, 15, 127, 128, 129, 134, 144, 164, 172, 174, 176, 178, 180, 182, 338, 334, 336, 21, 440, 327, 31, 39, 20, 22, 30, 449, 248, 252, 262, 257, 267, 272, 251, 292, 297, 282, 94, 186, 197, 195, 198, 375, 224, 383, 504, 325, 449, 69, 481, 465, 342, 340, 287, 786, 26, 761, 33, 44, 4759, 3547, 504, 1034, 520, 247, 104, 34], 0] += Cinfl_now[:, 0]
		# account for continuous extraction of chamber air
		if (self.wall_on > 0): # if wall on
			...
			dd[0:-num_comp, 0][df_indx] -= y[0:-num_comp, 0][df_indx]*1.*self.dil_fac_now
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((205563+jac_mod_len))
		...
		part_eff = np.zeros((128348))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:46672:8] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((40838))
		# effect of gas on gas 
		wall_eff[0:17502:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:17502:3] = +kimt[num_asb+wsb, :] 
