...
# File Created at 2021-09-01 13:41:14.804745
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	H2Oi, dil_fac, RO2_indx, comp_namelist):
	...
	def dydt(t, y): # define the ODE(s)
		...
		# account for components with constant influx
		dd[[0, 3, 18], 0] += Cinfl_now[:, 0]
	...
	def jac(t, y): # define the Jacobian
		...
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:340:17] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			...
			wall_eff[0:40:2] = -kw # effect of gas on gas 
			wall_eff[1:40:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[40:80:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[40+1:80:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
