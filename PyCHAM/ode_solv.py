...
# File Created at 2023-02-22 19:08:03.202736
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		part_eff = np.zeros((12))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:6:2] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((12))
		# effect of gas on gas 
		wall_eff[0:6:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:6:2] = +kimt[num_asb+wsb, :] 
