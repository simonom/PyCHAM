...
# File Created at 2023-10-18 07:11:50.775718
...
def ode_solv(y, integ_step, rrc, 
	...
	def jac(t, y): # define the Jacobian
		...
		part_eff = np.zeros((128))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:64:2] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((224))
		# effect of gas on gas 
		wall_eff[0:96:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:96:3] = +kimt[num_asb+wsb, :] 
