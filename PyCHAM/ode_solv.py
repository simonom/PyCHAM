...
# File Created at 2022-08-19 16:56:16.490811
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:2637:3] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff[0:1758:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:1758:2] = +kimt[num_asb+wsb, :] 
