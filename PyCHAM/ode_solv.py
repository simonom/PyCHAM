...
# File Created at 2022-10-26 17:18:49.472889
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		part_eff = np.zeros((154))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:56:8] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((49))
		# effect of gas on gas 
		wall_eff[0:21:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:21:3] = +kimt[num_asb+wsb, :] 
