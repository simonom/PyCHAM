...
# File Created at 2022-05-09 12:18:05.712264
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def dydt(t, y): # define the ODE(s)
		...
		dd[[1, 20, 10, 29, 2, 3, 53], 0] += Cinfl_now[:, 0]
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((3434+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((1272))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:636:2] = -kimt.sum(axis=0) # effect of gas on gas
