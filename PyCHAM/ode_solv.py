...
# File Created at 2024-07-17 14:13:06.225996
...
def ode_solv(y, integ_step, rrc, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((66641+jac_mod_len))
		...
		part_eff = np.zeros((57327))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:19503:33] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((2364))
		# effect of gas on gas 
		wall_eff[0:1182:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:1182:2] = +kimt[num_asb+wsb, :] 
