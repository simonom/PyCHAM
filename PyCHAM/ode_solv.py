...
# File Created at 2025-09-09 12:18:27.696816
...
def ode_solv(y, integ_step, rrc, 
	...
	# self.dil_fac_now - dilution factor for chamber (fraction of chamber air removed/s)
	...
	def dydt(t, y): # define the ODE(s)
		...
		# rate of loss of gas-phase reactants
		gpr_lr = np.array((loss.sum(axis = 1))[self.uni_y_rind_g])[:, 0]
		# register loss of reactants
		dd[self.uni_y_rind_g, 0] -= gpr_lr
		# gain of products
		...
		# rate of gain of gas-phase products
		gpr_gr = np.array((loss.sum(axis = 1))[self.uni_y_pind_g])[:, 0]
		dd[self.uni_y_pind_g, 0] += gpr_gr
		
		# account for components with fixed gas-phase concentration
		dd[[self.obs_comp_i], 0] = 0.
		...
		if (self.odsw_flag == 0): # if water solver not used 
			# dilute gas-phase water 
			dd[H2Oi, 0] -= y[H2Oi, 0]*self.dil_fac_H2Og_now
			# dilute particle-phase water 
			dd[num_comp+H2Oi::num_comp, 0] -= y[num_comp+H2Oi::num_comp, 0]*self.dil_fac_now
		...
		csum = ((ymat[0:num_asb, :].sum(axis=1)-ymat[0:num_asb, self.seedi].sum(axis=1)-ymat[0:num_asb, self.H2Oi])+((ymat[0:num_asb, self.seedi]*self.core_diss).sum(axis=1)).reshape(-1)+((ymat[0:num_asb, self.H2Oi]*self.H2O_diss_const_org)).reshape(-1)).reshape(-1, 1)
		...
		dd = dd.flatten()
		nzindx = dd != 0.
		# zero rates of change for components
		# with constant gas-phase concentration
		for rowi in self.conCindxn:
			dd[rowi] = 0.
		# account for components with fixed gas-phase concentration
		# by ensuring their rate of change is 0
		dd[self.obs_comp_i] = 0.
		return (dd)
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((9570+jac_mod_len))
		...
		part_eff = np.zeros((8479))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:2919:21] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		csum = ymat.sum(axis=1)-ymat[:, self.seedi].sum(axis=1)-ymat[:, self.H2Oi]+(ymat[:, self.seedi]*self.core_diss).sum(axis=1)+(ymat[:, self.H2Oi]*self.H2O_diss_const_org)
		...
		wall_eff = np.zeros((556))
		# effect of gas on gas 
		wall_eff[0:278:2] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:278:2] = +kimt[num_asb+wsb, :] 
		...
		# zero partial derivative rates of change for components
		# with constant gas-phase concentration
		for rowi in self.conCindxn:
			data[rowvals == rowi] = 0.
		# account for components with fixed gas-phase concentration
		# by ensuring their partial derivative rate of change is 0
		for rowi in self.obs_comp_i:
			data[rowvals == rowi] = 0.
		# create Jacobian
