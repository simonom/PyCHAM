...
# File Created at 2023-07-09 17:58:57.102551
...
def ode_solv(y, integ_step, rrc, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((108014+jac_mod_len))
		...
		part_eff = np.zeros((24172))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:12086:2] = -kimt[0:num_asb, :].sum(axis=0) # effect of gas on gas
		...
		wall_eff = np.zeros((42301))
		# effect of gas on gas 
		wall_eff[0:18129:3] = -np.sum(kimt[num_asb::, :], axis=0) 
		for wsb in range(int(self.wall_on)): # wall bin loop
			if (self.Cw[wsb, 0] > 0.):
				# effect of gas on wall 
				wall_eff[wsb+1:18129:3] = +kimt[num_asb+wsb, :] 
