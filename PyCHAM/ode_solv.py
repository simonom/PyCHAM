...
# File Created at 2021-10-27 19:25:37.931626
...
def ode_solv(y, integ_step, rindx, pindx, rstoi, pstoi, 
	...
	def jac(t, y): # define the Jacobian
		...
		data = np.zeros((28482+jac_mod_len))
		...
		# gas-particle partitioning
		part_eff = np.zeros((16653))
		if (sum(N_perbin[:, 0]) > 0.): # if any particles present 
			part_eff[0:6405:5] = -kimt.sum(axis=0) # effect of gas on gas
		...
		if (Cw > 0.):
			wall_eff = np.zeros((5124))
			wall_eff[0:2562:2] = -kw # effect of gas on gas 
			wall_eff[1:2562:2] = +kw # effect of gas on wall 
			# effect of wall on gas
			wall_eff[2562:5124:2] = +kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
			# effect of wall on wall
			wall_eff[2562+1:5124:2] = -kw*(Psat[0,:]*act_coeff[0, :]/Cw) 
