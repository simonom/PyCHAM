...
def nuc(sumt, new_part_sum1, n0, y, MW, num_comp, Varr, x, new_partr, 
	MV, siz_str, rbou, Vbou, num_sb, self):
	...
	# self.nucv1/v2/v3 - parameter values for nucleation equation
	# self.nuc_comp - index of the nucleating component
	...
	# self - PyCHAM class
	# --------------------------------------------------------------
	# density of components in g/cm3 in 1D array	
	rho = np.squeeze(self.y_dens*1.e-3) 

	# total number concentration of particles that should have been 
	# produced through nucleation by now given by integration:

	# Gompertz function (for asymmetric sigmoid curve): 
	# https://en.wikipedia.org/wiki/Gompertz_function
	nsum1 = self.nucv1*np.exp(self.nucv2*(np.exp(-sumt/self.nucv3)))
	...
	# scale new_partr to convert from cm to um
	sbi = sum(new_partr*1.e4 > rbou[1::]) 
	...
	# remove from gas-phase (# molecules/cm3 (air)), note 
	# commented out as violation of
...
# 	y[self.nuc_comp[0]] -= nuc_conc1
	...
	# convert from cm3 to um3), note, consider all size bins since 
	# if full-moving used,
	# the smallest and largest size bin will be affected
	ypsb = y[num_comp:num_comp*(num_sb+1)].reshape(num_sb, 
		num_comp, order = 'C')
	
	ish = (n0[:, 0] > 0) # index of size bins containing particles
	if (sum(ish) > 0): # if any bins contain particles
		Varr[ish] = ((ypsb[ish, :]/(si.N_A*n0[ish, :]))*(
		MV[:, 0].reshape(1, -1)*1.e12)).sum(axis = 1)
