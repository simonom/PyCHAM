...
# 		ax0.set_ylim([8.0e-6, 1.0e0])
		...
		ax0.set_xlabel(r'$\rm{D_p}$ ($\rm{\mu}$m)', fontsize=12)
		ax0.set_ylabel(r'$\rm{\beta_{T}$ $(s^{-1})}$', fontsize=12)
...
# ensure path allows us to see wallloss module (assumes this test module called whilst
# inside Unit_Testing folder)
...
fig, (ax0) = plt.subplots(1, 1, figsize=(6,7.6))

test_wallloss(Pn, Cn, Gi, eta_a, Dp, MW, Varr, sbn, nc, TEMP, t, 
			inflectDp, pwl_xpre, pwl_xpro, inflectk, ChamR, Rader, testf, p_char, 
			e_field, fig, ax0) # call testing function
...
inflectDp = 1.0e-6 # diameter at which inflection in deposition rate occurs
pwl_xpre = 1.0
pwl_xpro = 1.0 
inflectk = 2.0e-5
p_char = -1e6
