'''module for calculating photolysis rates for MCM (Master Chemical Mechanism) equations using customised actinic flux file'''

import numpy as np

def MCM_photo(fname, J, TEMP, act_flux_path):
	...
	# act_flux_path - path of actinic flux file, 'no' if none given
	
	# --------------------------------------------------------------
	# open wavelengths (nm) we have total actinic flux for (photon/cm2/nm/s)
	# from chamber
	f = open(str(act_flux_path), 'r')
