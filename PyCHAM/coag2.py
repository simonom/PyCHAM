...
# 	sbr_m[np.triu_indices(sbrn, 1, m=sbn)] = 0.0
	...
	if (coag_on == 0):
		Beta = Beta*0.
	if (testf == 2): # if testing the cpagulation equations below, fix the kernels
	...
	for sbi in range(sbn):
		...
		if (siz_str == 1):
			...
			if (sbi > 0):
				...
				frac_sub_in = 1.-(sbVi[0, sbi]-coagV)/(sbVi[0, sbi]-sbVi[0, sbi-1])
			...
			if (sbi < sbn-1):
				...
				frac_exc_in = 1. - (coagV-sbVi[0, sbi])/(sbVi[0, sbi+1]-sbVi[0, sbi])				
		...
		if (siz_str == 0): # moving-centre structure
		...
		# In the numerator of Eq. 15.12 (Jacobson 2005) the i index can only rise as high as
		# k-1, therefore, zero any combinations of i=k with other particle sizes to produce
		# a particle in size bin k 
		volind[sbi, :] = 0.
		...
		if (siz_str == 0): # moving-centre structure
			...
			volind = (coagV[sbi, :] >= sbbound[0, sbi])
			# lose half the number of particles of sbi coagulating with itself to give a 
		...
		# consistency with particle number gain calculation above 
		volind[sbi, :] = 0.
		...
		# fraction of particles lost going to each size bin
		frac_loss = ((Beta[sbi, :]*volind*num_partj[0, :])/
		...
		if (testf == 2):
			...
			print(str('total particle number concentration gained by size bin '+str(sbi)+': '+str(sum(num_contr)*0.5)))
			print(str('particle number concentration gained by size bin ' +str(sbi)+' from each coagulating pair (in ascending order of size bin): '+str(numsum_ind*0.5)))
