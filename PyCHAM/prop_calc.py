...
def prop_calc(comp_list, Pybel_objects, TEMP, H2Oi, num_speci, Psat_water, vol_Comp, 
				volP, testf, corei, pconc, umansysprop_update, core_dens, spec_namelist,
				ode_gen_flag, nuci, nuc_comp, num_asb):
	...
	Psat = np.zeros((1, num_speci)) # species' vapour pressure
	...
	Psat_Pa = np.zeros((1, num_speci)) # for storing vapour pressures in Pa (Pa)
	Psat_Pa[0, :] = Psat[0, :]
