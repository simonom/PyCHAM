...
# created at 2024-02-07 07:59:01.320324
...
def evaluate_rates(RO2, H2O, TEMP, time, M, N2, O2, Jlen, NO, HO2, NO3, sumt, self):
	...
	try:
		...
		gprn += 1 # keep count on reaction number
		rate_values[13] = 6.3e-16*numpy.exp(-580/TEMP)*0.06
		gprn += 1 # keep count on reaction number
		rate_values[14] = 6.3e-16*numpy.exp(-580/TEMP)*0.06
		gprn += 1 # keep count on reaction number
		rate_values[15] = 6.3e-16*numpy.exp(-580/TEMP)*0.06
		gprn += 1 # keep count on reaction number
		rate_values[16] = 6.3e-16*numpy.exp(-580/TEMP)*0.06
	except:
		erf = 1 # flag error
