...
# created at 2023-05-16 09:39:46.308688
...
def evaluate_rates(RO2, H2O, TEMP, time, M, N2, O2, Jlen, NO, HO2, NO3, sumt, self):
	...
	try:
		...
		rate_values[0] = 1.2e-12*numpy.exp(490/TEMP)*0.65
		gprn += 1 # keep count on reaction number
		rate_values[1] = 1.2e-12*numpy.exp(490/TEMP)*0.65
		gprn += 1 # keep count on reaction number
		rate_values[2] = 1.2e-12*numpy.exp(490/TEMP)*0.65
		gprn += 1 # keep count on reaction number
		rate_values[3] = 1.2e-12*numpy.exp(490/TEMP)*0.65
	except:
		erf = 1 # flag error
