...
def ode_gen(int_tol, rowvals, num_comp, num_asb, testf, self):
	...
	f.write('	act_coeff, kelv_fac, kimt, num_asb,\n')
	f.write('	jac_mod_len, jac_part_hmf_indx, rw_indx, N_perbin, jac_part_H2O_indx,\n')
	f.write('	H2Oi, self):\n')
	f.write('\n')
	f.write('	# inputs: -------------------------------------\n')
	f.write('	# y - initial concentrations (# molecules/cm^3)\n')
	...
	f.write('	# self.Psat - pure component saturation vapour pressures (# molecules/cm^3)\n')
	f.write('	# self.Cw - effective absorbing mass concentration of wall (# molecules/cm^3) \n')
	...
	f.write('	# N_perbin - number concentration of particles per size bin (# particles/cm^3)\n')
	...
	if (testf > 0):
		...
		f.write('	csum = ((ymat.sum(axis=1)-ymat[:, self.seedi]-ymat[:, self.H2Oi])+((ymat[:, self.seedi]*self.core_diss))+((ymat[:, self.H2Oi]*self.H2O_diss_const_org))).reshape(-1, 1)\n')
	...
	if (num_asb > 0 or self.wall_on > 0): # include gas-particle partitioning in ode solver
		...
		if (num_asb > 0):
			...
			f.write('		csum = ((ymat[0:num_asb, :].sum(axis=1)-ymat[0:num_asb, self.seedi].sum(axis=1)-ymat[0:num_asb, self.H2Oi])+((ymat[0:num_asb, self.seedi]*self.core_diss).sum(axis=1)).reshape(-1)+((ymat[0:num_asb, self.H2Oi]*self.H2O_diss_const_org)).reshape(-1)).reshape(-1, 1)\n')
	...
	if (num_asb > 0): # include gas-particle partitioning in Jacobian
		...
		f.write('		# total particle-phase concentration per size bin (molecules/cm3 (air))\n')
		f.write('		csum = ymat.sum(axis=1)-ymat[:, self.seedi].sum(axis=1)-ymat[:, self.H2Oi]+(ymat[:, self.seedi]*self.core_diss).sum(axis=1)+(ymat[:, self.H2Oi]*self.H2O_diss_const_org)\n')
		f.write('		\n')
