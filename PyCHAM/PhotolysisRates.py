...
import shutil
from MCM_photo import MCM_photo

def PhotolysisCalculation(time, lat, lon, TEMP, act_flux_path, DayOfYear):
...
# 		z.extractall('PyCHAM/MCMphotofiles') # creates folder and stores files there
		Photo_name = os.listdir(path='PyCHAM/MCMphotofiles')
		# call on MCM_photo to process photolysis files and estimate J values
		J = MCM_photo(str('PyCHAM/MCMphotofiles/' + Photo_name[0]), J, TEMP, act_flux_path)
