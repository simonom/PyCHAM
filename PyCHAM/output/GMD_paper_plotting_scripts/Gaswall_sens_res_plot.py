...
# 1.0e-3 /s and Cw was set to 7.85e-6 g/m3 (air), as these values are comparable to those
# for particulate matter (for particulate matter, total concentration (core, water and
# SOA summed) was around 7.85e-6 g/m3 (air))

cwd = os.getcwd() # get current working directory
output_by_sim = str(cwd + '/PyCHAM/output/isoprene_scheme_MCM/Gaswall_sens_Cw7p85e-6_kw1e2')
...
# ----------------------------------------------------------------------------------------
# third case is high gas-wall partitioning, where kw set to 
# 1.0e-3 /s and Cw was set to 7.85e-3 g/m3 (air)
cwd = os.getcwd() # get current working directory
output_by_sim = str(cwd + '/PyCHAM/output/isoprene_scheme_MCM/Gaswall_sens_Cw7p85e-6_kw1e2')
...
# ----------------------------------------------------------------------------------------
# fourth case is high gas-wall partitioning, where kw set to 
# 1.0e2 /s and Cw was set to 7.85e-56 g/m3 (air)

cwd = os.getcwd() # get current working directory
output_by_sim = str(cwd + '/PyCHAM/output/isoprene_scheme_MCM/Gaswall_sens_Cw1e-4_kw1e2')
...
ax0.plot(t_array/3600.0, SOA1, linewidth=3, label=r'$k_{w}=1\rm{x}10^{-3}\, \mathrm{s^{-1}}$, $C_{w}=8\rm{x}10^{0}\, \mathrm{\mu g \, m^{-3}}$')
ax0.plot(t_array/3600.0, SOA2, linewidth=3, label=r'$k_{w}=1\rm{x}10^{-3}\, \mathrm{s^{-1}}$, $C_{w}=8\rm{x}10^{3}\, \mathrm{\mu g \, m^{-3}}$')
ax0.plot(t_array/3600.0, SOA3, '--', linewidth=3, label=r'$k_{w}=1\rm{x}10^{2}\, \mathrm{s^{-1}}$, $C_{w}=8\rm{x}10^{0}\, \mathrm{\mu g \, m^{-3}}$')
...
num_comp = int((const['num_comp'])[0]) # number of species
y_mw = const['mw']
y_MV = const['mv']
