...
def write_dydt_rec(): # define function
	...
	# suggest 1 Tab
	f.write('def dydt_rec(y, rindx, rstoi, reac_coef, pindx, pstoi, nprod, step, nreac, num_sb, num_comp, pconc, core_diss, Psat, kelv_fac, kimt, act_coeff, dydt_erh_flag, H2Oi, wat_hist, self):\n')
	f.write('	\n')
	...
	f.write('		if (any(self.kw > 1.e-10)): \n')
	f.write('			# concentration at walls (# molecules/cm3 (air)) \n')
	f.write('			Csit = y[num_comp*(num_sb+1)::].reshape(len(self.Cw, num_comp)) \n')
	f.write('			Csit = (Psat[0, :]*(Csit/self.Cw)*act_coeff[0, :])\n')
