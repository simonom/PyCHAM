...
def write_dydt_rec(self): # define function
	...
	f = open(self.PyCHAM_path + '/dydt_rec.py', mode='w')
	f.write('##########################################################################################\n')
	f.write('#                                                                                        #\n')
	...
	f.write('			\n')
	f.write('		# now estimate and record tendency to change due\n		# to particle- and wall-partitioning  \n')
	...
	f.write('		if (hasattr(self, \'sim_ci_file\') and step < ci_array.shape[0]):\n')
