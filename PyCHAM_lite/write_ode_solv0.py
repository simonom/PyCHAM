...
def ode_gen(con_infl_indx, int_tol, rowvals, num_comp, 
	...
	f.write('	# self.jac_den_indx - index of component denominators for Jacobian\n')
	...
	f.write('	# self.y_rindx_g - index of y relating to reactants for reaction rate \n')
	...
	f.write('	# 	calculation\n')
	f.write('	# self.rr_arr_p_g - index for reaction rates to allow reactant loss\n')
	f.write('	# 	calculation\n')
	...
	f.write('		print("{:.54f}".format(float(y[-3])))\n')
	f.write('		print(self.ode_cnt)\n')
	f.write('		self.ode_cnt += 1\n')
	f.write('		\n')
	...
	if (self.eqn_num[0] > 0): # if gas-phase reactions present
		...
		f.write('		data = rr[self.rr_arr_p_g]*self.pstoi_flat_g # prepare gain values\n')
		f.write('		# convert to sparse matrix\n')
	...
	if (self.eqn_num[1] > 0): # if particle-phase reactions present
		...
		f.write('		# empty array to hold relevant concentrations for\n')
		f.write('		# reaction rate coefficient calculation\n')
		f.write('		# tile aqueous-phase reaction rate coefficients over particle size bins\n')
		f.write('		rr_aq = np.tile(rrc[self.rindx_g.shape[0]:self.rindx_g.shape[0]+self.rindx_aq.shape[0]], num_asb)\n')
		...
		f.write('		# convert to sparse matrix\n')
		f.write('		loss = SP.csc_matrix((data[0, :], self.y_rind_aq, self.reac_col_aq))\n')
		f.write('		# register loss of reactants\n')
		f.write('		dd[self.uni_y_rind_aq, 0] -= np.array((loss.sum(axis = 1))[self.uni_y_rind_aq])[:, 0]\n')
		f.write('		# gain of products\n')
		f.write('		data = rr[self.rr_arr_p_aq]*self.pstoi_flat_aq # prepare gain values\n')
		f.write('		# convert to sparse matrix\n')
	...
	if (self.eqn_num[2] > 0): # if surface (e.g. wall) reactions present
		...
		f.write('		# empty array to hold relevant concentrations for\n')
		f.write('		# reaction rate coefficient calculation\n')
		f.write('		# tile surface (e.g. wall) reaction rate coefficients over surfaces\n')
		f.write('		rr_su = np.tile(rrc[-self.rindx_su.shape[0]::], self.wall_on)\n')
		f.write('		# prepare surface (e.g. wall) concentrations\n')
		...
		f.write('		# convert to sparse matrix\n')
		f.write('		loss = SP.csc_matrix((data[0, :], self.y_rind_su, self.reac_col_su))\n')
		f.write('		# register loss of reactants\n')
		f.write('		dd[self.uni_y_rind_su, 0] -= np.array((loss.sum(axis = 1))[self.uni_y_rind_su])[:, 0]\n')
		f.write('		# gain of products\n')
		f.write('		data = rr[self.rr_arr_p_su]*self.pstoi_flat_su # prepare gain values\n')
		f.write('		# convert to sparse matrix\n')
		f.write('		loss = SP.csc_matrix((data[0, :], self.y_pind_su, self.prod_col_su))\n')
		f.write('		# register gain of products\n')
	...
	if (self.eqn_num[1] > 0): # if particle-phase reactions present
		...
		f.write('		\n')
		f.write('		for i in range(self.rindx_g.shape[0], self.rindx_g.shape[0]+self.rindx_aq.shape[0]): # aqueous-phase reaction loop\n')
		f.write('			# reaction rate (# molecules/cm3/s)\n')
		...
		f.write('			rr = (np.tile(rr, int(self.njac_aq[aqi, 0]/(num_asb)))).flatten(order=\'C\')\n')
		f.write('			# prepare Jacobian inputs\n')
	...
	if (self.eqn_num[2] > 0): # if surface (e.g. wall) reactions present
		f.write('		n_sur = self.nreac_su.shape[0] # number of surface (e.g. wall) reactions \n')
		f.write('		sui = 0 # surface (e.g. wall) reaction counter\n')
		f.write('		\n')
		f.write('		for i in range(self.rindx_aq.shape[0], self.rindx_aq.shape[0]+self.rindx_su.shape[0]): # surface (e.g. wall reaction loop\n')
		f.write('			# reaction rate (# molecules/cm3/s)\n')
		f.write('			rr = rrc[i]*(y[self.rindx_su[sui::n_sur, 0:self.nreac_su[sui]], 0].prod(axis=1))\n')
		f.write('			# spread along affected components\n')
		f.write('			rr = rr.reshape(-1, 1)\n')
		f.write('			rr = (np.tile(rr, int(self.njac_su[sui, 0]/(self.wall_on)))).flatten(order=\'C\')\n')
		f.write('			# prepare Jacobian inputs\n')
		f.write('			jac_coeff = np.zeros((self.njac_su[sui, 0]))\n')
		f.write('			nzi = (rr != 0)\n')
		f.write('			jac_coeff[nzi] = (rr[nzi]*((self.jac_stoi_su[sui, 0:self.njac_su[sui, 0]])[nzi])/\n')
		f.write('				((y[self.jac_den_indx_su[sui, 0:self.njac_su[sui, 0]], 0])[nzi]))\n')
		f.write('			# stack size bins\n')
		f.write('			jac_coeff = jac_coeff.reshape(int(self.wall_on), int(self.njac_su[sui, 0]/(self.wall_on)), order=\'C\')\n')
		f.write('			data[self.jac_indx_su[sui::n_sur, 0:(int(self.njac_su[sui, 0]/(self.wall_on)))]] += jac_coeff\n')
		f.write('			\n')
		f.write('			sui += 1 # keep count on aqueous-phase reactions \n')
	f.write('		\n')
