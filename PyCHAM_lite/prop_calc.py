...
def prop_calc(H2Oi, num_comp, Psat_water, vol_Comp, 
	...
	if (umansysprop_update == 1):
		...
		if os.path.isdir(self.PyCHAM_path[
			0:(self.PyCHAM_path.index('PyCHAM_lite')-1)] + '/umansysprop'): 
			...
			# needed
			shutil.rmtree(self.PyCHAM_path[
			0:(self.PyCHAM_path.index('PyCHAM_lite')-1)] + 
			'/umansysprop', ignore_errors=False, onerror=handleRemoveReadonly)
		
		git_url = 'https://github.com/loftytopping/UManSysProp_public.git'
		Repo.clone_from(git_url, (self.PyCHAM_path[
			0:(self.PyCHAM_path.index('PyCHAM_lite')-1)] + '/umansysprop'))
		
		# now check on whether import pybel written in data/__init__.py
		f_init = open(str(self.PyCHAM_path[
			0:(self.PyCHAM_path.index('PyCHAM_lite')-1)] + 
			'/umansysprop/umansysprop/data/__init__.py'), mode='r')
		lines = f_init.readlines()
		if 'import pybel' in lines[21]:
			lines[21] = 'import openbabel.pybel as pybel\n'	
			f_init = open(str(cwd + 
				'/umansysprop/umansysprop/data/__init__.py'), mode='w')
			f_init.writelines(lines)
		f_init.close()

	# point to umansysprop folder
	# address for updated version
	sys.path.insert(1, (self.PyCHAM_path[
		0:(self.PyCHAM_path.index('PyCHAM_lite')-1)] + '/umansysprop'))
	...
	if (self.pars_skip == 3):
		# get vapour pressures at 298.15 K (Pa)
		load_path = str(self.PyCHAM_path + '/prop_store/pure_component_saturation_vapour_pressures_at_298p15K_Pa.npy') # path
	...
	#save_path = str(self.PyCHAM_path + '/prop_store/' + 
